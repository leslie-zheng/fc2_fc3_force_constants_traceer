...
This script is used to trace the second and third order force constants
to get the relatiuonship between the bond strength VS. distance
"""
...
if mode == 2:
    ...
    pair_distance = np.zeros((num_atom_super, num_atom_super),dtype=float)
        ...
        for j in range(i):
            if projection is not None:
                ...
                get_equivalent_smallest_vectors(i, j,
                                                ...
                                                symprec=1e-5)
                vectors = np.dot(vectors, primitive.get_cell())
                project_vector = np.eye[projection]
    ...
    if options.is_relative:
        ...
        self_trace = 1.0
    for i in np.arange(num_atom_super):
        d1 = pair_distance[index_super, i]
        for j in np.arange(num_atom_super):
            d2 = pair_distance[index_super, j]
            d3 = pair_distance[i, j]
            ...
            if dirt is not None:
                if options.rotation_matrix is not None:
                    fc_rot = np.einsum("ij, kl, mn, jln->ikm", rotmat, rotmat, rotmat, fc[index_super, i, j])
                    fc_temp = fc_rot[dirt[0], dirt[1], dirt[2]]
                else:
    ...
    if options.xreverse is not None:
        ind = np.argmax(force_in_range)
        print("Max force constant within the given distance range %f to %f" % tuple(reverse_range))
    ...
    for i, c1 in enumerate(chemicals):
        for j in range(i+1):
            ...
            filename2 = 'relationship'
            with open(filename2, 'a+') as f:
                f.write("%15s %15s" %("distance", symbol+'-'+c1+'-'+c2))
    with open(filename2, 'a+') as f:
        f.write("\n")
            #print("%15s %15s" %("distance", symbol+'-'+c1+'-'+c2), end='')
    #print()
    lenght=0
    while True:
        all_empty=True
        for i, c1 in enumerate(chemicals):
            for j in range(i+1):
                ...
                if len(dist)<=lenght:
                    ...
                    with open(filename2, 'a+') as f:
                        #f.write("\n")
                        f.write("%15.7f %15.7f" %tuple(dist[lenght]))
        with open(filename2, 'a+') as f:
             f.write("\n")
                    #print("%15.7f %15.7f" %tuple(dist[lenght]), end='')
        #print()
        lenght+=1
        if all_empty:
            break
    ...
    for i in np.arange(len(chemicals)):
        for j in np.arange(i+1):
            marker = next(markers)
            if options.is_log:
                pl.scatter(distance[chemap[i]][:,chemap[j]], np.abs(fc_relative_trace[chemap[i]][:,chemap[j]]),
                           label=symbol+"-"+chemicals[i]+"-"+chemicals[j], color=colors[i*len(chemicals)+j], marker=marker)
            else:
                pl.scatter(distance[chemap[i]][:, chemap[j]], fc_relative_trace[chemap[i]][:, chemap[j]],
                           label=symbol + "-" + chemicals[i] + "-" + chemicals[j], color=colors[i * len(chemicals) + j],
                           marker=marker)
